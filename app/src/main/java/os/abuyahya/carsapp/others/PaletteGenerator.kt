package os.abuyahya.carsapp.others
...
object PaletteGenerator {
    ...
    fun extractColorsFromBitmap(bitmap: Bitmap): Map<String, String> {
        return mapOf(
            "vibrant" to parseColorVibrantSwatch(
                color = Palette.from(bitmap).generate().vibrantSwatch
            ),
            "darkVibrant" to parseColorDarkVibrantSwatch(
            ...
            "onDarkVibrantTitle" to parseBodyColor(
                color = Palette.from(bitmap).generate().darkVibrantSwatch?.titleTextColor
            )
        )
    }

    private fun parseColorVibrantSwatch(color: Palette.Swatch?): String {
    ...
    private fun parseColorDarkVibrantSwatch(color: Palette.Swatch?): String {
        return if (color != null) {
            val parsedColor = Integer.toHexString(color.rgb)
            "#$parsedColor"
        } else {
